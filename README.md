...
ПЕРЕД ТЕСТИРВОАНИЕМ НУЖНО ОБЯЗАТЕЛЬНО ЗАМЕНИТЬ ССЫЛКУ НА ТЕСТОВУЮ БД В 
ФАЙЛЕ ***server.py***!!!
...
Хостинг сайта был произведен с помощью сервиса Glitch (Хостинг Glitch прекратил существование, и ссылка сейчас н еработает :( )
Приложение доступно (уже недоступно) по такому адресу: [online-store-15.glitch.me]()
